...
@RestController
@Slf4j
public class EventControllerHistory {
    ...
    ResponseEntity createEvent(){
        ...
        URI createdUri = linkTo(methodOn(EventControllerHistory.class).createEvent()).slash("{id}").toUri();
        return ResponseEntity.created(createdUri).build();
    }

    @PostMapping(value = "/event02")
    ResponseEntity createEvent02(@RequestBody Event event){
        URI createdUri = linkTo(methodOn(EventControllerHistory.class).createEvent02(event)).slash("{id}").toUri();
        event.setId(10);
...
//    }
    ...
    ResponseEntity createEvent03(@RequestBody Event event){
        ...
        URI createdUri = linkTo(methodOn(EventControllerHistory.class).createEvent03(event)).slash(createdEvent.getId()).toUri();
        return ResponseEntity.created(createdUri).body(event);
    }
...
//    }
    ...
    ResponseEntity createEvent04(@RequestBody EventDto eventDto){
        ...
        URI createdUri = linkTo(methodOn(EventControllerHistory.class).createEvent04(eventDto)).slash(createdEvent.getId()).toUri();
    ...
    * */
    ...
    public EventControllerHistory(EventRepository eventRepository, ModelMapper modelMapper, EventValidator eventValidator){
        this.eventRepository = eventRepository;
        this.modelMapper = modelMapper;
    ...
    ResponseEntity createEvent08(@RequestBody @Valid EventDto eventDto, Errors errors){
        ...
        ControllerLinkBuilder selfLinkBuilder = linkTo(methodOn(EventControllerHistory.class).createEvent08(eventDto, errors));
        URI createdUri = selfLinkBuilder.toUri();

        EventRepresentationModel eventRepresentationModel = new EventRepresentationModel(event);
        eventRepresentationModel.add(selfLinkBuilder.withSelfRel());
        eventRepresentationModel.add(selfLinkBuilder.slash(createdEvent.getId()).withRel("query-event"));
        eventRepresentationModel.add(selfLinkBuilder.slash(createdEvent.getId()).withRel("update-event"));
    ...
    public ResponseEntity createEvent09(@RequestBody @Valid EventDto eventDto, Errors errors){
        ...
        ControllerLinkBuilder selfLinkBuilder = linkTo(methodOn(EventControllerHistory.class).createEvent09(eventDto, errors));
        URI createdUri = selfLinkBuilder.toUri();
